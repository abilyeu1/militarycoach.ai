...
import { CareerWizardChatBarProps } from "@/types/tools.interface";

const ChatBar: FC<CareerWizardChatBarProps> = ({
  questions,
  ...
  isGenerating,
  setErrorMessage,
  isSkeletonCodeVisible, // setIsSkeletonCodeVisible
}: any) => {
      ...
      >
        ...
        <div className="flex flex-col justify-between">
          <div className="">
            ...
            <div
              className={`scrollbar  overflow-auto overflow-x-hidden px-1 md:px-5 ${
                history.length > 1
                  ? "max-h-[57vh] min-h-[57vh]"
            ...
            >
              <div className="my-5 flex">
                <div
                  className={`mb-2 ml-1 mr-0 flex-1 rounded-lg p-1 md:ml-4 md:mr-0 md:p-5 lg:relative `}
                >
                  {history.slice(2).map((item: any, index: number) => (
                    ...
                    >
                      {item.content && (
                        <pre
                          className={`relative my-5 whitespace-pre-wrap ${
                            item.role === "user"
                              ? "relative my-2 rounded-xl border border-primary p-2 text-left text-primary md:max-w-[60%] lg:p-3 lg:text-right"
                        ...
                        >
                          ...
                          <ReactMarkdown
                            className="markdown-content relative max-w-full"
              ...
              {!startConversation && (
                <div className="mb-10 flex  flex-col md:items-end">
                  {questions.map((ques: any, index: number) => (
                    <pre
                      key={index}
                      className="md:max-w-[60%]w-full my-2 cursor-pointer whitespace-pre-wrap rounded-xl border border-primary p-2 text-left text-[13px] text-primary md:text-[16px] lg:p-3 lg:text-right"
                      onClick={() => handleQuestionClick(ques)}
                    >
                      {ques}
                    </pre>
                  ))}
                  {/* Render Skills Gap Analysis Tool separately */}
                </div>
              )}
            </div>
            ...
            <div>
              ...
              {history.length > 1 && (
                <div className="mx-4 my-3 flex justify-end rounded-lg bg-[#F4F4F4] p-2 py-4">
                  <p className="font-normal">
                    Found your dream career? Use the{" "}
